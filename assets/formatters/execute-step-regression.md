<driftlock_prompt kind="formatter" name="execute-step-regression" version="1">
  ...
  <output schema="assets/schemas/execute-step.schema.json">
    ...
    <required_fields>success (boolean), summary (string), details (string), filesTouched (string[]), filesWritten (string[]), patch (string), mode ("fix_regression")</required_fields>
  </output>
  ...
  <hard_constraints>
    ...
    <constraint>If patch is non-empty, it MUST be a unified diff with repo-relative paths. If you are omitting patch content, set patch to an empty string.</constraint>
    <constraint>You may leave filesTouched/filesWritten empty (Driftlock will fill them from git). If you provide non-empty lists, filesWritten must include every file changed by the patch and filesTouched must include all filesWritten.</constraint>
